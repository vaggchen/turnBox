<template>
  <div class="cnt h-full  grid gap-2 grid-cols-5">
    <div v-for="(item, index ) in lists" class="box rounded-md" :class="!item ? 'bg-blue-300' : 'bg-red-300'"
...
const boxClick = (index) => {
  ...
  botInxdex < lists.value.length && (lists.value[botInxdex] = changeVal(botInxdex))
  ...
  rightInxdex < lists.value.length && (parseInt(index / rowLen) === parseInt(rightInxdex / rowLen)) && (lists.value[rightInxdex] = changeVal(rightInxdex))
